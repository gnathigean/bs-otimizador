...
def obter_hwid():
    """Gera o HWID único do computador através do UUID da Placa-Mãe."""
...
class TelaLogin(ctk.CTk):
    def __init__(self):
        ...
        self.centralizar_janela(420, 580)
        ...
        # Checkboxes (Salvar Senha e Login Automático)
        ...
        self.btn_registo = ctk.CTkButton(self, text="CRIAR NOVA CONTA", width=280, height=45, font=("Roboto Medium", 13), fg_color="transparent", border_width=1, border_color="#48485E", text_color="#EDEDF0", hover_color="#1E1E26", corner_radius=8, command=self.efetuar_registo)
        self.btn_registo.pack(pady=5)

        # Painel de Licença (Fica Oculto por Padrão)
    ...
    def efetuar_registo(self):
        ...
        try:
            sucesso, msg = banco_dados.registrar_usuario(user, senha)
            if sucesso: messagebox.showinfo("Bem-vindo!", msg)
            else: messagebox.showerror("Erro no Registo", msg)
        except Exception as e:
            messagebox.showerror("Erro Interno", f"Falha na comunicação com o banco de dados.\n{e}")
